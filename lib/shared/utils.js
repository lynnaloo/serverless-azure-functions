...
var Utils = /** @class */ (function () {
    ...
    Utils.getFunctionMetaData = function (functionName, serverless) {
        var config = serverless.service;
        var bindings = [];
        ...
        var functionObject = serverless.service.getFunction(functionName);
        var handler = functionObject.handler;
        var events = functionObject["events"];
        var params = {
            functionJson: null
        };
        var parsedBindings = bindings_1.BindingUtils.getBindingsMetaData(serverless);
        ...
        for (var eventsIndex = 0; eventsIndex < events.length; eventsIndex++) {
            ...
            bindingUserSettings = {};
            var azureSettings = events[eventsIndex][constants_1.constants.xAzureSettings];
            var bindingTypeIndex = bindingTypes.indexOf(bindingType);
            var bindingUserSettingsMetaData = bindings_1.BindingUtils.getBindingUserSettingsMetaData(azureSettings, bindingType, bindingTypeIndex, bindingDisplayNames);
            ...
            if (azureSettings) {
                for (var azureSettingKeyIndex = 0; azureSettingKeyIndex < Object.keys(azureSettings).length; azureSettingKeyIndex++) {
                    var key = Object.keys(azureSettings)[azureSettingKeyIndex];
        ...
        var _a = Utils.getEntryPointAndHandlerPath(handler, config), handlerPath = _a.handlerPath, entryPoint = _a.entryPoint;
        if (functionObject["scriptFile"]) {
            handlerPath = functionObject["scriptFile"];
        }
        return {
            entryPoint: entryPoint,
            handlerPath: path_1.relative(functionName, handlerPath),
            params: params
        };
    };
    Utils.getEntryPointAndHandlerPath = function (handler, config) {
        ...
        var handlerPath = ((handlerSplit.length > 1) ? handlerSplit[0] : handler)
            + constants_1.constants.runtimeExtensions[config.provider.functionRuntime.language];
        return {
            entryPoint: entryPoint,
    ...
    Utils.getIncomingBindingConfig = function (functionConfig) {
        return functionConfig.events.find(function (event) {
            var settings = event["x-azure-settings"];
            return settings && (!settings.direction || settings.direction === "in");
        });
    };
    Utils.getOutgoingBinding = function (functionConfig) {
        return functionConfig.events.find(function (event) {
            var settings = event["x-azure-settings"];
