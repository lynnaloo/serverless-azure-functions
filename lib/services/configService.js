...
var runtimeVersions_json_1 = __importDefault(require("./runtimeVersions.json"));
/**
...
var ConfigService = /** @class */ (function () {
    ...
    ConfigService.prototype.getArtifactName = function (deploymentName) {
        ...
        var _a = config_1.default.naming.suffix, deployment = _a.deployment, artifact = _a.artifact;
     ...
     */
    ConfigService.prototype.getFunctionRuntime = function () {
    ...
    ConfigService.prototype.setDefaultValues = function (config) {
        var _a = config_1.default.defaults, awsRegion = _a.awsRegion, region = _a.region, stage = _a.stage, prefix = _a.prefix;
    ...
    ConfigService.prototype.initializeConfig = function (service) {
        ...
        var _a = config.provider, prefix = _a.prefix, region = _a.region, stage = _a.stage, subscriptionId = _a.subscriptionId, tenantId = _a.tenantId, appId = _a.appId, deployment = _a.deployment, runtime = _a.runtime;
        var options = {
            config: config,
        ...
        config.provider.deployment = __assign({}, config_1.default.deploymentConfig, deployment);
        config.provider.functionRuntime = this.getRuntime(runtime);
        return config;
    };
    ConfigService.prototype.getRuntime = function (runtime) {
        guard_1.Guard.null(runtime, "runtime", "Runtime version not specified in serverless.yml");
        var versionMatch = runtime.match(/([0-9]+\.)+[0-9]*x?/);
        var languageMatch = runtime.match(/nodejs|python/);
        var versionInput;
        var languageInput;
        ...
        var targetedVersionRegex = new RegExp(("^" + versionInput)
            .replace(".", "\.")
            .replace("x", "[0-9]+"));
        var matchingVersions = runtimeVersions_json_1.default[languageInput]
            .filter(function (item) { return item.version.match(targetedVersionRegex); })
        ...
        var version = matchingVersions.sort(semver_1.default.rcompare)[0];
        var language = {
            "nodejs": serverless_1.SupportedRuntimeLanguage.NODE,
            "python": serverless_1.SupportedRuntimeLanguage.PYTHON
        }[languageInput];
        return {
            language: language,
            version: version
    ...
    ConfigService.prototype.getRuntimeErrorMessage = function (language) {
        if (language) {
            return "Supported versions for " + language + " are: " + runtimeVersions_json_1.default[language]
                .map(function (item) { return item.version; })
                .join(",");
        }
        return "Supported versions: " + JSON.stringify(runtimeVersions_json_1.default, null, 2);
    };
    /**
